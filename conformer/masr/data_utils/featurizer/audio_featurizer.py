...
class AudioFeaturizer(object):
    ...
    def _compute_linear(samples, sample_rate, frame_shift=5.0, frame_length=10.0, eps=1e-14):
    ...
    # Fbank
    # 定义一个私有方法 _compute_fbank，用于计算滤波器组特征（FBank）
    # 这个方法可能是一个类的一部分，用于处理音频信号
    def _compute_fbank(self,
                       # 输入的音频样本，预期是一个NumPy数组
                       samples,
                       # 音频的采样率
                       sample_rate,
                       # 梅尔滤波器的数量，默认为161
                       ...
                       # frame_shift=5,
                       # frame_length=15,
                       # 帧移，即相邻帧之间的时间间隔，默认为10毫秒
                       frame_shift=10,
                       # 帧长，即每个帧的时间长度，默认为25毫秒
                       frame_length=25,
                       # 用于增加随机噪声的值，以扩展动态范围
                       dither=1.0,
                       # 是否处于训练模式，默认为False
        ...
        # 根据是否训练，设置dither的值
        dither = dither if train else 0.0
        # 将输入的音频样本转换为一个PyTorch张量，并增加一个维度，形成一个"批次"
        waveform = torch.from_numpy(np.expand_dims(samples, 0)).float()
        # 计算Fbank
        # 调用fbank函数计算梅尔频率滤波器组特征
        # 这个函数的具体实现没有给出，但我们可以假设它接受以下参数：
        # waveform - 音频张量
        # num_mel_bins - 梅尔滤波器的数量
        # frame_length - 帧长
        # frame_shift - 帧移
        # dither - 噪声值
