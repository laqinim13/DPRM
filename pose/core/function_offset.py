...
from pose.utils.transforms import flip_back, flip_back_offset
from pose.utils.vis import save_debug_images
from pose.utils import log
...
def train(config, train_loader, model, criterion, optimizer, writer_dict,epoch=0, device="cuda:0"):
    ...
    if config.MODEL.OUT_TYPE == 'offset':
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        if config.MODEL.OUT_TYPE == 'gaussian':
            loss = criterion(output, target, target_weight)
        elif config.MODEL.OUT_TYPE == 'offset':
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            ...
            if config.TEST.FLIP_TEST:
                ...
                if config.TEST.SHIFT_HEATMAP and config.MODEL.OUT_TYPE == 'gaussian':
