...
def train(config, train_loader, model, teacher_model, criterion, kd_criterion, kd_weight, optimizer, epoch, writer_dict,
          device):
...
def validate(config, val_loader, model, criterion, writer_dict, device, epoch):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    output_dir = config.work_dir
