...
def set_device_memory(gpu_fraction=0.90, eager_execution=True):
    ...
    gpu_id = tf.cfg.experimental.list_physical_devices('GPU')
    if gpu_id:
        try:
            # Currently, memory growth needs to be the same across GPUs
            for gpu in gpu_id:
                tf.cfg.experimental.set_memory_growth(gpu, True)
                print("use gpu id:{}".format(gpu.name))
            logical_gpus = tf.cfg.experimental.list_logical_devices('GPU')
