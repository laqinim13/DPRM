...
class PoseAction(PoseEstimation):
    def __init__(self, config, threshold=0.0, device="cuda:0"):
        """
        :param model_path: 模型文件，支持ONNX，TFlite,PB和h5等格式的模型
        :param input_size: 模型输入input size
        :param threshold: 关键点置信度
        """
        super(PoseAction, self).__init__(config, threshold=threshold)
    ...
    def start_capture(self, video_path, save_root, detect_freq=1):
        ...
        :return:
        """
        file_utils.create_dir(save_root)
        result_json = []
        ...
        while True:
            ...
            if count % detect_freq == 0:
                ...
                frame = image_utils.resize_image(frame, resize_height=500)
        ...
        video_cap.release()
        file_utils.write_json_path(os.path.join(save_root, "{}_{}.json".format(basename, time)), result_json)
    ...
    def task(self, bgr_image):
        ...
        key_points, kp_scores, body_rects = self.detect_image(bgr_image, threshold=self.threshold)
        points, scores = key_points[0], kp_scores[0]
        points = points / (width, height)
        result = {"points": points.tolist(), "scores": scores.tolist()}
        self.draw_result(bgr_image, key_points, body_rects, kp_scores, delay=15)
        return result
    ...
    def detect_video_dir(self, video_root, save_root, waitKey=0):
        ...
        if os.path.isdir(video_root):
            video_list = file_utils.get_files_list(video_root, postfix=["*.mp4", "*.MP4", "*.avi", "*.AVI",
                                                                             "*.MOV", "*.mov", "*.rmvb", "*.RMVB",
                                                                             "*.3gp"])
        else:
...
if __name__ == '__main__':
    ...
    hp = PoseAction(config=val_config.custom_coco_finger4_model_mbv2, device="cuda:0")
    ...
    video_path = "/home/data/dataset3/dataset/finger_keypoint/pen_v1/action/video_crop"
    # video_path = "/home/data/dataset3/dataset/finger_keypoint/pen_v1/video_crop"
    save_root = "/home/data/dataset3/dataset/finger_keypoint/pen_v1/action/video_json2"
