...
def cv_show_image(title, image, use_rgb=False, delay=5):
    """
    调用OpenCV显示图片
    :param title: 图像标题
...
def draw_key_point_in_image(image, key_points, pada_string, pointline=[], boxes=[], colors=None, vis_id=False, thickness=2):
    """
    :param image:
    ...
    x_string = boxes[0][0]
    y_string = boxes[0][1]
    position = (x_string, y_string)
    font_face = cv2.FONT_HERSHEY_SIMPLEX  # 添加字体类型参数
    ...
    for p in range(nums):
        ...
        image = cv2.putText(image, pada_string, org=position,fontFace=font_face, fontScale=3, color=(255,255,255),thickness=4)
        #image = cv2.putText(image, pada_string, org=(100,100),fontFace=font_face, fontScale=3, color=(255,255,255),thickness=4)
    return image
