...
from pose.utils.voc_parser import VOCDataset, ConcatDataset
...
class VOCKeyPointDataset(VOCDataset):
    ...
    def __getitem__(self, index):
        ...
        angles = image_utils.data_normalization(angles, omin=0, omax=1, imin=-180, imax=180)
...
def draw_pair_points_bbox_angle(image, bboxes, labels, angels=None, keypoints_pair=None):
    ...
    if angels is not None:
        ...
        angels = image_utils.data_normalization(angels, omin=-180, omax=180, imin=0, imax=1)
    pointline = [(0, 1)]
    angle = None
    for i in range(len(labels)):
        box = bboxes[i, :]
        image = image_utils.draw_image_bboxes_text(image, [box], [labels[i]])
        ...
        if keypoints_pair is not None:
            ...
            image = image_utils.draw_key_point_arrowed_in_image(image, [[point1, point0]], pointline=pointline)
            print(angle)
        if angels is not None:
            angle = angels[i]
        ...
        image = image_utils.draw_key_point_arrowed_in_image(image, [[start, end]],
                                                            pointline=pointline,
                                                            color=(255, 0, 0))
        vis_image = image_utils.resize_image(image, 1000)
        image_utils.cv_show_image("Det", vis_image, waitKey=0)


def angles_normalization():
    angle = -180
    norm_angele = image_utils.data_normalization(angle, omin=0, omax=1, imin=-180, imax=180)
    unnorm_angele = image_utils.data_normalization(norm_angele, omin=-180, omax=180, imin=0, imax=1)
...
if __name__ == "__main__":
    ...
    isshow = True
    data_root = "/media/data/data1/git/python-learning-notes/dataset/finger/"
    image_dir = data_root + "images"
    # data_root = "/home/data/dataset3/dataset/wider_face_add_lm_10_10/"
