...
from pose.augment.augment_landm_crop import OrientationModel
...
from pose.core import udp_offset

logger = logging.getLogger(__name__)


class JointsDataset(Dataset):
    def __init__(self, cfg, root, image_set="", is_train=False, transform=None):
        # self.num_joints = 0
        # self.pixel_std = 200
        # self.flip_pairs = []
        ...
        self.OUT_TYPE = cfg.MODEL.OUT_TYPE
    ...
    def __getitem__(self, idx):
        ...
        if self.OUT_TYPE == 'gaussian':
            ...
            Exception("Error:".format(self.OUT_TYPE))
        # trans = get_affine_transform(center, scale, rot, self.image_size)
        # input = cv2.warpAffine(data_numpy, trans, (int(self.image_size[0]), int(self.image_size[1])),
        #                        flags=cv2.INTER_LINEAR)
        # for i in range(self.num_joints):
    ...
    def generate_target(self, joints, joints_vis):
        ...
        if self.OUT_TYPE == 'gaussian':
            ...
            raise Exception("out_type:{}".format(self.OUT_TYPE))
